...
<div class="container">
    ...
    UNION ALL SELECT COALESCE((SELECT string_agg('<tr><td><span class="badge b-blue">CHECKSUM</span></td><td>Data Integrity</td><td>Status: <span class="code">'||status||'</span> | Failures: '||fails||'</td></tr>','') FROM v_checksum_status),'')
    UNION ALL SELECT '</table></div>'
    
    UNION ALL SELECT '<div class="card" id="infra"><div class="card-header"><svg class="icon"><path d="M15 3s-3.3 0-6 3c-2 2.5-3 6-3 6l5 5s3.5-1 6-3c3-2.7 3-6 3-6l-5 5z"></path></svg> <span data-i18n="h_infra">Infrastructure & Replication (PG19+ Enhanced)</span></div><table><tr><th>Status</th><th>Component</th><th>Metric / Details</th></tr>'
    ...
    UNION ALL SELECT COALESCE((SELECT string_agg('<tr><td><span class="badge b-red">RECOVERY</span></td><td>Standby State</td><td>Status: <span class="code">'||status||'</span> | File: '||redo_wal_file||'</td></tr>','') FROM v_recovery_state),'')
    ...
    UNION ALL SELECT '<div class="card" id="maint"><div class="card-header"><svg class="icon"><path d="M3 13h18M6 13v6a2 2 0 002 2h8a2 2 0 002-2v-6M12 3v10"></path></svg> <span data-i18n="h_maint">Maintenance & Parallelization (PG19+)</span></div><table><tr><th>Alert</th><th>Object</th><th>Metric / Status</th></tr>'
    ...
    UNION ALL SELECT COALESCE((SELECT string_agg('<tr><td><span class="badge b-blue">PARALLEL AV</span></td><td class="code">'||schemaname||'.'||relname||'</td><td>Utilization: <span class="code">'||utilization||'%</span></td></tr>','') FROM v_autovacuum_parallel),'')
    ...
    UNION ALL SELECT '<div class="card" id="activity"><div class="card-header"><svg class="icon"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"></path></svg> <span data-i18n="h_activity">Real-time Activity & Lock Contention (PG19+)</span></div><table><tr><th>PID / Type</th><th>Status / Waits</th><th>Wait / Latency</th><th>Query / Analysis</th></tr>'
    ...
    UNION ALL SELECT COALESCE((SELECT string_agg('<tr><td><span class="badge b-orange">'||locktype||'</span></td><td class="code">Waits: '||total_waits||'</td><td>'||avg_ms||'ms</td></tr>','') FROM v_lock_stats LIMIT 10),'')
    UNION ALL SELECT '</table></div>'
